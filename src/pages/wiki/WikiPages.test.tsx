...
/** Renders the real routes at `path` with `articles` already in localStorage. */
async function renderWiki(path = '/wiki', articles: WikiArticle[] = []) {
  ...
  // Every wiki page renders nothing until the async load from storage has finished.
...
describe('Wiki', () => {
  ...
  describe('search', () => {
    it.each([
      ...
      ['tag', 'devops', ['Docker']],
    ])('finds by %s while typing', async (_field, query, expected) => {
    ...
    it('keeps the query when the user comes back from an article', async () => {
      ...
      expect(screen.getByPlaceholderText('Поиск по статьям...')).toHaveValue('docker')
      expect(cardTitles()).toEqual(['Docker'])
    })
  })
  ...
  describe('tags', () => {
    ...
    it('combines a tag filter with a search query', async () => {
      ...
      expect(cardTitles()).toEqual(['TypeScript'])
    })
  ...
  describe('article page', () => {
    ...
    it('lists related articles with shared tags, but not unrelated ones or itself', async () => {
      ...
      const related = screen.getByRole('region', { name: 'Связанные статьи' })
      expect(
        within(related)
          .getAllByRole('link')
  ...
  describe('navigation', () => {
    it('shows a sidebar on desktop with tag counts, quick search and a create link', async () => {
      ...
      expect(within(sidebar).getByRole('link', { name: /#devops/ })).toHaveTextContent('1')
    ...
    it('uses a slide-out menu on narrow screens', async () => {
      ...
      await user.click(within(menu).getByRole('link', { name: /#devops/ }))
