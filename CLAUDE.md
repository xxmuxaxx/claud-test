...
- Fully client-side, no API/store: `hooks/useNeuralNetworkLab` (a `useReducer` plus a `setInterval` tick of ~30 ms, running only while training) drives everything. The math is a from-scratch MLP in `src/lib/neuralNetwork/`, written to be read top to bottom, no ML libraries, deliberately unoptimised: `neuralNetwork.ts` is the core (`forward` → `datasetLoss` → `backpropagate` → `applyGradients`, plus `trainEpoch`; networks are immutable), `activations.ts` (sigmoid/relu/tanh/linear + derivatives), `datasets.ts` (XOR/AND/OR truth tables and the scattered `clusters`/`circle` sets, generated from a fixed seed), `random.ts` (seeded `Random`, always passed in), `trainingSession.ts` (network + dataset + epoch/loss/history, `runIteration`), `visualization.ts` (diagram layout, decision-grid sampling, plane↔pixel mapping).
- Model: 2 inputs (a point on the plane) → 1–4 hidden layers of 1–8 neurons (the chosen activation) → 1 sigmoid output. Loss is MSE, training is full-batch gradient descent, so one epoch = one weight update. `ModelSettings` (dataset, layers, activation) rebuild the network with new random weights (Reset does too); `TrainingSettings` (learning rate, total epochs, epochs per step) apply live without a reset. One "iteration" = `epochsPerStep` epochs: that is what a tick and the Step button run. The status (`ready/training/paused/finished`) is derived, not stored.
- UI is in `components/neuralNetwork/`, all plain SVG (no canvas): `NetworkDiagram` (neuron fill = activation for the _probe_ point, line width/opacity = |weight|, blue = positive, orange = negative; see `colors.ts`), `DecisionBoundary` (40×40 grid of rects; a click sets the probe, and `TruthTable` for the logic sets does the same), `LossChart`, `Metrics`, `TrainingControls`, `SettingsPanel`. Colours use opacity instead of theme tokens, so they work in both themes.
- Tests: the maths (`lib/neuralNetwork/*.test.ts`) includes a finite-difference check of backprop and the XOR scenario; `pages/NeuralNetworkPage.test.tsx` pins `Math.random` (it seeds the weights) and uses fake timers for Start/Pause.
